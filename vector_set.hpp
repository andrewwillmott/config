...
// vector_set.hpp
//
// Defines vector_set -- a set based on std::vector. Avoids an allocation
...
namespace HL
{
    ...
    public:
        ...
        typedef std::vector<Key, Alloc>        super;
        ...
        template<class K> iterator       find(const K& key);        // find by variant key, e.g., const char* vs. vector_set<std::string>
        template<class K> const_iterator find(const K& key) const;  // find by variant key, e.g., const char* vs. vector_set<std::string>


        iterator_and_added  insert(const Key& key);  // reproduce STL semantics: if the key was already present, its value is not updated & added=false
    ...
    VECTOR_SET_TMPL template<class K> inline typename VECTOR_SET::iterator VECTOR_SET::find(const K& key)
    {
        KeyLess less;
        iterator it = lower_bound(this->begin(), this->end(), key, less);
    ...
    VECTOR_SET_TMPL template<class K> inline typename VECTOR_SET::const_iterator VECTOR_SET::find(const K& key) const
    {
        KeyLess less;
        const_iterator it = lower_bound(this->begin(), this->end(), key, less);
